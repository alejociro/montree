...
<template>
    <div class="flex min-h-screen flex-col bg-background text-foreground">
        ...
        <footer class="bg-[#2B3B2E] text-white">
            <div class="mx-auto grid w-full max-w-7xl gap-8 px-4 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-3 lg:px-8">
                <div class="space-y-4">
                    <h3 class="text-sm font-semibold uppercase tracking-wider">Información de Contacto</h3>
                    <ul class="space-y-3 text-sm text-white/70">
                        <li class="flex items-start gap-2">
                            <MapPin class="mt-0.5 size-4 shrink-0" />
                            <span>{{ configuration?.contact_info?.address ?? 'Calle 123, Siempre Viva' }}</span>
                        </li>
                        <li class="flex items-center gap-2">
                            <Phone class="size-4 shrink-0" />
                            <span>{{ configuration?.contact_info?.phone ?? '+57 3009910019' }}</span>
                        </li>
                        <li class="flex items-center gap-2">
                            <Mail class="size-4 shrink-0" />
                            <span>{{ configuration?.contact_info?.email ?? 'contacto@ecotravel.com' }}</span>
                        </li>
                    </ul>
                </div>

                <div class="space-y-4">
                    <h3 class="text-sm font-semibold uppercase tracking-wider">Enlaces Rápidos</h3>
                    <ul class="space-y-3 text-sm text-white/70">
                        <li>
                            <span class="transition hover:text-white">Sobre Nosotros</span>
                        </li>
                        <li>
                            <span class="transition hover:text-white">Términos y condiciones</span>
                        </li>
                    </ul>
                </div>

                <div class="space-y-4">
                    <h3 class="text-sm font-semibold uppercase tracking-wider">Síguenos en Redes Sociales</h3>
                    <div class="flex items-center gap-4">
                        <a :href="configuration?.social_links?.facebook ?? '#'" aria-label="Facebook" class="text-white/70 transition hover:text-white">
                        ...
                        <a :href="configuration?.social_links?.instagram ?? '#'" aria-label="Instagram" class="text-white/70 transition hover:text-white">
                            <svg class="size-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div class="border-t border-white/10">
                <div class="mx-auto flex w-full max-w-7xl items-center justify-center px-4 py-4 text-xs text-white/50 sm:px-6 lg:px-8">
