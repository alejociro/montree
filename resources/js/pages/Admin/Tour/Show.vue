...
type TourTab = 'summary' | 'passengers';

const activeTab = ref<TourTab>('summary');

const tabs = computed<Array<{ id: TourTab; label: string }>>(() => {
    const items: Array<{ id: TourTab; label: string }> = [
        { id: 'summary', label: t('Resumen') },
    ];

    if (canViewPassengers.value) {
        items.push({ id: 'passengers', label: t('Pasajeros') });
    }
...
<template>
    ...
    <div class="px-4 py-6 md:px-8">
        ...
        <nav
            ...
            :aria-label="$t('Secciones del tour')"
        >
            <button
                v-for="tab in tabs"
                :key="tab.id"
                ...
                class="-mb-px border-b-2 px-4 py-2.5 text-sm font-medium transition focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none"
                :class="
                    activeTab === tab.id
                        ...
                        {{
                            $t(
                                ':confirmed confirmadas · :pending pendientes · :cancelled canceladas',
                                {
                                    ...
                                    pending:
                                        props.stats.bookings.pending_payment,
